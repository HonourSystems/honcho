 ...
 *
 * Routes Honcho's LLM calls through `claude -p` (the CLI in print mode),
 * using the host's interactive Claude Code auth (Max subscription).
 *
 * The Agent SDK doesn't support OAuth tokens, so we use the CLI directly.
 * Each request spawns `claude -p` with no tools and no session persistence.
 *
 * Runs on the host (not in Docker). Docker containers reach it
...
async function callClaude(
  ...
  const settingsPath = import.meta.dir + "/clean-settings.json";
  const args = [
    "-p",
    "--output-format", "text",
    "--model", model,
  ...
  // Log for debugging
  console.log(`[proxy] settings: ${settingsPath}`);
  console.log(`[proxy] args: ${args.join(" ").substring(0, 200)}`);

  const proc = Bun.spawn(["claude", ...args], {
    stdin: "pipe",
    stdout: "pipe",
    stderr: "pipe",
    cwd: "/tmp/claude-proxy-sandbox",  // empty dir — no project context loaded
  });
  ...
  // Only fail if exit code is non-zero AND we got no stdout.
  // SessionEnd hook failures go to stderr but the response is still valid.
...
const server = Bun.serve({
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/") {
      return Response.json({ status: "ok", proxy: "claude-cli" });
    }
...
console.log(`[claude-code-proxy] using claude CLI (-p mode)`);
